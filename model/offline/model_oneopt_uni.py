...
def compute_loss(batch, beta):
    ...
    # Compute policy loss
    a_loss = -torch.mean(torch.sum(a_dist.log_prob(A), dim=-1)) / T
...
def compute_test_loss(batch, beta):
    s0, S, A, sT = batch["s0"], batch["state_sequence"], batch["action_sequence"], batch["sT"]
    B, T, _ = S.shape
...
def eval_epoch(val_loader, q_phi, pi_theta, p_psi, p_omega, beta, device):
    ...
    for batch in val_loader:
        ...
        terms = compute_test_loss(batch, beta)
        loss = terms["loss"]
...
def skill_model_training_with_val(
    ...
    q_phi, pi_theta, p_psi, p_omega, beta,
    lr=5e-5,
...
for beta in betas:
    ...
    wandb.init(
        ...
        name=f"antmaze-medium-detached-uni-epoch{epochs}-beta{beta}",
        config=dict(
    ...
    save_path = f"checkpoints/{beta}"
    os.makedirs(os.path.join(save_path, "epochs"), exist_ok=True)
    curves = skill_model_training_with_val(save_path, train_loader, val_loader, q_phi, pi_theta, p_psi, p_omega, beta, epochs=epochs, lr=lr, steps=1)
