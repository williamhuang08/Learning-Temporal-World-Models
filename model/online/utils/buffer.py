...
class DreamerSubtrajDataset(Dataset):
    """
    Subtrajectories for RSSM + causal EM training:
      - obs_seq:      o_0 … o_H   (H+1 observations)
      - act_seq:      a_0 … a_{H-1} (H actions)
      - reward_seq:   r_0 … r_{H-1} (H per-step rewards from the environment)
      - goal_xy:      desired_goal xy at window start, shape [2]
      - min_goal_dist: min Euclidean distance to goal over the window
    ...
    def __init__(self, minari_dataset, H, episode_ids, stride=1, max_step_size=0.5):
        ...
        for ep_idx, ep in enumerate(minari_dataset.iterate_episodes()):
            ...
            for t in range(0, Ltot - H, stride):
                ...
                rew_window = rew[t : t + H].astype(np.float32)     # [H]
                ...
                if not np.all(dxy <= max_step_size):
                    self.removed_items.append(
                        (obs_window, act_window, rew_window, goal_xy, min_goal_dist)
                    )
                    continue
    ...
    def __getitem__(self, i):
        obs_window, act_window, rew_window, goal_xy, min_goal_dist = self.items[i]
        ...
        return {
            ...
            "reward_seq": torch.as_tensor(rew_window, dtype=torch.float32),       # [H]
...
def dreamer_collate(batch):
    return {
        ...
        "reward_seq": torch.stack([b["reward_seq"] for b in batch], 0),
...
def compute_stats(ds):
    ...
    all_obs = []
    for obs_window, _, _, _, _ in ds.items:
